...
class _CreateBookingScreenState extends State<CreateBookingScreen> {
  ...
  void initState() {
    super.initState();
    // Nếu có dates được truyền vào, set luôn
    if (widget.preSelectedStartDate != null) {
      _startDate = widget.preSelectedStartDate;
    }
    if (widget.preSelectedEndDate != null) {
      _endDate = widget.preSelectedEndDate;
    }
  ...
  Future<void> _loadPreSelectedRoom() async {
    try {
      ...
      // Nếu đã có cả start và end date, tự động load available rooms
      if (_startDate != null && _endDate != null) {
        _loadAvailableRooms();
      }
    } catch (e) {
      _showError('Lỗi khi tải thông tin phòng: ${e.toString()}');
    }
  }
  ...
  Future<void> _createBooking() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        await _showBookingSuccessDialog(booking);

        // Quay về trang bookings
        if (mounted) {
          Navigator.pop(context, true); // true = đã tạo booking mới
        }
      }
    } catch (e) {
      setState(() {
